/*
Copy from https://www.shadertoy.com/view/lls3zf
*/
...
// License. Created by S.Guillitte
...
vec2 noise2b(in vec2 p) // Simplex Noise from IQ
{
  ...
  vec3 nx = h * h * h * h *
            vec3(dot(a, hash2(i + 0.0)), dot(b, hash2(i + o)),
                 dot(c, hash2(i + 1.0)));
  vec3 ny = h * h * h * h *
            vec3(dot(a, hash2(i + 4.5)), dot(b, hash2(i + o + 4.5)),
                 dot(c, hash2(i + 1.0 + 4.5)));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  float c = length(p) / 7.;
  c = clamp(pow(c, 3.), 0., 1.);
  vec3 col = texture2D(iChannel0, fract(uv / 20. + p / 45.)).rgb;
